...
class minikube(ShutItModule):
	...
	def build(self, shutit):
		if not shutit.command_exists('minikube'):                                                                                                     
		...
		if not shutit_send_and_get_output('''minikube status | grep 'Does Not Exist' | wc -l | awk '{print $1}' ''') == '1'                           
			shutit.send('./minikube start')                                                                                                           
		if not shutit.command_exists('kubectl'):                                                                                                      
