...
class swarm(ShutItModule):
  ...
  config.vm.define "swarm3" do |swarm3|
    ...
    swarm3.vm.hostname = "swarm3.vagrant.test"
		...
		#shutit.send('docker-machine create -d generic --generic-ip-address ' + swarm2_ip + ' --engine-env GODEBUG=netdns=cgo --swarm --swarm-master --swarm-discovery token://' + token + ' swarm1')
		#shutit.send('docker-machine create -d generic --generic-ip-address ' + swarm3_ip + ' --engine-env GODEBUG=netdns=cgo --swarm --swarm-discovery token://' + token + ' swarm2')
		shutit.send('docker-machine create -d generic --generic-ip-address ' + swarm1_ip + ' --engine-env GODEBUG=netdns=cgo swarm1')
		shutit.send('docker-machine create -d generic --generic-ip-address ' + swarm2_ip + ' --engine-env GODEBUG=netdns=cgo swarm2')
		shutit.send('docker-machine create -d generic --generic-ip-address ' + swarm3_ip + ' --engine-env GODEBUG=netdns=cgo swarm3')
